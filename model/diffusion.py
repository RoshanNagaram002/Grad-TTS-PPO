...
class Diffusion(BaseModule):
    ...
    @torch.no_grad()
    def reverse_diffusion(self, z, mask, mu, n_timesteps, stoc=False, spk=None, all_log_probs=False):
        h = 1.0 / n_timesteps
        xt = z * mask
        ret_log_probs = []
        ...
        for i in range(n_timesteps):
            ...
            if stoc:  # adds stochastic term
                ...
                std_dev_t = torch.sqrt(noise_t * h)
                dxt_stoc = dxt_stoc * std_dev_t
                dxt = dxt_det + dxt_stoc

                log_prob = (
                    -((dxt.detach() - dxt_det) ** 2) / (2 * (std_dev_t**2))
                    - torch.log(std_dev_t)
                    - torch.log(torch.sqrt(2 * torch.as_tensor(math.pi)))
                )
                # mean along all but batch dimension
                log_prob = log_prob.mean(dim=tuple(range(1, log_prob.ndim)))
                ret_log_probs.append(log_prob)
    ...
    def reverse_diffusion_ppo(self, z, mask, mu, n_timesteps, advantages, base_log_probs, optimizer, stoc=False, spk=None, clip_range=1e-4):
        assert stoc
        h = 1.0 / n_timesteps
        xt = z * mask
    ...
    def forward(self, z, mask, mu, n_timesteps, stoc=False, spk=None, all_log_probs=False):
        return self.reverse_diffusion(z, mask, mu, n_timesteps, stoc, spk, all_log_probs=all_log_probs)
